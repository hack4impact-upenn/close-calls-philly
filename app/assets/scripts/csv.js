function downloadCSV(isAdmin) {
    ...
    var csv = [
            ...
            'NUMBER OF BICYCLES,DESCRIPTION,INJURIES,LICENSE PLATES,PICTURE URL']
    ];
    if (isAdmin) {
    ...
    markersDisplayedOnMap.forEach(function(marker) {
        ...
        var line = [marker.incidentDate, marker.locationName, marker.pedestrianNum,
                    marker.automobileNum, marker.bicycleNum, marker.description,
                    marker.injuries, license_plates, marker.pictureUrl];
        if (isAdmin) {
